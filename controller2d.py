    ...
    def get_lookahead_index(self, lookahead_distance):
        ...
        for i in range(len(self._waypoints)):
            ...
            if dist < min_dist:
                min_dist = dist
                min_idx = i
                #waypoints 다 검사해서 제일 짧은 dist의 waypoints와의 절대값을 x,y로 저장

        total_dist = min_dist# total_dist로 저장
        lookahead_idx = min_idx
        for i in range(min_idx + 1, len(self._waypoints)):
            if total_dist >= lookahead_distance: #내가 2미터 까지 보기로했으면 나머지 앞쪽은 다 버림
                break
            total_dist += np.linalg.norm(np.array([
                    self._waypoints[i][0] - self._waypoints[i-1][0],
                    self._waypoints[i][1] - self._waypoints[i-1][1]]))
            lookahead_idx = i
            # 만약 total_dist가 3,2라면 waypoints 끝까지 안가고 내가 보기로한 범위내의 가장 먼 idx return
